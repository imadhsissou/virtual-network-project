...
function etc_hosts {
  ...
  for CNAME in `docker ps |awk '$NF !~/^NAMES$/ && $NF !~/^host[0-9]$/ && $NF !~/-server$/ {print $NF}'`; do
    for IP_ADDR in `docker exec $CNAME ifconfig |grep "inet addr:" |awk ' /[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/ {print $2}' | awk -F":" ' /^addr:10\./ {print $2}'`; do
      IP=$IP_ADDR
      NAME=$CNAME
  ...
  cat ./base/dnsmasq.conf | docker exec -i backbone sh -c 'cat > /etc/dnsmasq.conf'
